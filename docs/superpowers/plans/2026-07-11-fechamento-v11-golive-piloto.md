...
> ## STATUS DE EXECUÇÃO (2026-07-11, goal "finalizar o plano")
> **Concluído ao vivo:** F0.1 (rota pública do tunnel — criada no painel CF via Chrome principal; validada: /meta-leads/health 200, verify 403, irmãos intactos) · F1 (e2e nativo+receiver+CAPI — 2 bugs de produção achados e corrigidos: WAF do GHL/User-Agent + telefone dummy; review aprovado; teste de regressão `fc09cb1`) · F2.4/F5.1 (healthcheck.sh, testado ao vivo) · F3.1 (saúde live: 4 checks ✓ + lint 4/0) · F3.2 (**MERGE em main `534b8ad`**) · F3.3 (tag `v1.1.0`; versões 1.1.0 consistentes; install real com sessão nova = Flávio).
> **Bloqueado — só o Flávio resolve:** F0.2 (App Secret — reautenticação por senha no painel Meta; confirmado ao vivo, cancelado por regra de segurança). Consequência: **F2** (Verify/Save do webhook + assinar campo leadgen + test lead público) espera o App Secret — NÃO ligar antes, senão leads reais da página batem 403. **F4** (piloto) espera decisões de negócio do Flávio (clínica/página/WhatsApp/workflow publicado) + F2. F5.2 (rotina news) = pós-piloto.
> **F2 reduzido a 1 comando** (`webhook-receiver/deploy/finish-golive.sh`, já na VPS + testado): configura a subscription do app (F2.1 Verify/Save + F2.2 campo leadgen) via API — dispensa painel/browser. Guard recusa com placeholder.
> **Próximo passo do Flávio (2 min):** no painel Meta revelar o App Secret (pede a senha dele) → `sed -i 's/^META_APP_SECRET=.*/META_APP_SECRET=<valor>/' /opt/meta-leads/.env && systemctl restart meta-leads && bash /opt/meta-leads/deploy/finish-golive.sh`. Isso completa F2 sozinho. Depois: test lead público (F2.3) + `crontab` do healthcheck (F2.4), ambos instruídos pelo próprio script. F4 (piloto) = decisões de negócio dele + F2 pronto.
