...
| 8 | `GET {page_id}/subscribed_apps` exige **page token**, não o token de system-user | Erro `100/190` subcode `2069032` ("É necessário um token de acesso à Página") | Derivar page token via `GET {page_id}?fields=access_token` antes de qualquer chamada a `subscribed_apps`/checagem de subscrição de `leadgen` |
